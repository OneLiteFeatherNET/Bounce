...
/**
 * Decides whether Bounce runs with LuckPerms.
 * <p>
...
public final class LuckPermsSupport {
    ...
    private static final AtomicBoolean BROKEN_PROVIDER_LOGGED = new AtomicBoolean(false);
    ...
    /**
     * Logs, once, that LuckPerms is on the class path but its provider never started - every
     * permission check fails closed instead of throwing.
     *
     * @param permission the permission node that triggered the failure
     * @param cause      the exception {@link net.luckperms.api.LuckPermsProvider#get()} threw
     */
    public static void noteBrokenProvider(String permission, Throwable cause) {
        if (BROKEN_PROVIDER_LOGGED.compareAndSet(false, true)) {
            LOGGER.error("LuckPerms is on the class path but its provider never started; denying '{}' and every "
                    + "subsequent permission check instead of throwing. This line only prints once.", permission, cause);
        }
    }
